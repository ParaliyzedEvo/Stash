    ...
    private suspend fun executeDownload(track: Track, preResolvedUrl: String?): TrackDownloadResult {
        ...
        if (forceLossless || losslessPrefs.enabledNow()) {
            ...
            if (!forceLossless && !losslessPrefs.youtubeFallbackEnabledNow()) {
                // Debug/beta builds carry no lossless credentials of their own —
                // LosslessAvailability sees no connected Qobuz account, no relay and
                // no custom endpoint, ARCOD_CONFIGURED is false, and amz needs none
                // but is proxy-outage-prone. Respecting "fallback off" here would
                // permanently strand every beta tester's tracks in
                // WAITING_FOR_LOSSLESS regardless of their toggle.
