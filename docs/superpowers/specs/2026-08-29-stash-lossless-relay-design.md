...
- **Plan A — client groundwork (Kotlin):** tokenless catalog, `QBDLX_CONFIGURED` redefinition + registry filters, `allDead()` redefinition, `QbdlxQobuzSource` restructuring, `LosslessRelayClient` + `LosslessConfigFetcher` (pointing at an empty list), strict-FLAC deferral reason, pasted-token migration. Ships as the groundwork release; independent of the server.
